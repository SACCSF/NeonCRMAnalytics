...
def get_accounts_additional_information(accountId, accountType):
    ...
    if accountType == "INDIVIDUAL":
        date = additional_information.get(
            "individualAccount.timestamps.createdDateTime"
        )
        if pd.isna(
            additional_information.get("individualAccount.primaryContact.gender")[0]
        ):
            gender = ""
        else:
            gender = additional_information.get(
                "individualAccount.primaryContact.gender"
            )[0]
        if pd.isna(
            additional_information.get("individualAccount.primaryContact.title")[0]
        ):
            title = ""
        else:
            title = additional_information.get(
                "individualAccount.primaryContact.title"
            )[0]
        if pd.isna(
            additional_information.get("individualAccount.primaryContact.employer")
        ):
            employer = ""
        else:
            employer = additional_information.get(
                "individualAccount.primaryContact.employer"
            )
        if pd.isna(
            additional_information.get("individualAccount.primaryContact.origin")
        ):
            origin = ""
        else:
            origin = additional_information.get(
                "individualAccount.primaryContact.origin"
            )
        if pd.isna(
            additional_information.get(
                "individualAccount.primaryContact.originCategory"
            )
        ):
            originCategory = ""
        else:
            originCategory = additional_information.get(
                "individualAccount.primaryContact.originCategory"
            )
        companyName = ""
        """
...
def print_all_accounts_to_csv():
    ...
    dataexport = pd.merge(
        accounts, df, on=["accountId"], how="outer", validate="one_to_one"
    )
